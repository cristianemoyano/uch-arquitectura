...
export default function ListAdminProducto() {
	...
	function sendData(sendProduct: any) {
		console.log("Anterior");
		console.log(sendProduct);
		router.push({
			pathname: "/admin/productForm",
			query: { product: sendProduct.id }
		});
	}
							...
							<table className="min-w-full leading-normal">
								...
								products.map((product: any, index:any) => {
									...
									<tr key={`product-${index}`}>
										...
										<td className="px-5 py-5 border-b border-gray-200 bg-white text-sm">
											<button  className="flex-no-shrink bg-green-500 px-5 ml-4 py-2 text-sm shadow-sm hover:shadow-lg font-medium tracking-wider border-2 border-green-500 text-white rounded-full" onClick={() => sendData(product)}>Editar</button>
										</td>
										<td className="px-5 py-5 border-b border-gray-200 bg-white text-sm">
